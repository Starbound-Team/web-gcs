    ...
    map = L.map('map', {
        ...
        contextmenuItems: [
          ...
          },
          {
            ...
            callback: function(e) {
              const { lat, lng } = e.latlng;
              sendAddWaypointCommand(lat, lng);
            }
          },
          {
            text: 'Return to home',
...
function sendAddWaypointCommand(lat, lng) {
    showToast("Adding waypoint...", "info");

    fetch(`${backendUrl}/api/mission_add_waypoint`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            // Optionally, add a visual marker for the waypoint on the map
            L.marker([lat, lng], {
                icon: L.divIcon({
                    className: 'waypoint-marker',
                    html: '<i class="fas fa-map-marker-alt" style="color: purple;"></i>' // Example icon
                })
            }).addTo(map).bindPopup(`Waypoint<br>Lat: ${lat.toFixed(6)}, Lon: ${lng.toFixed(6)}<br>Alt: ${alt.toFixed(2)}m`);
        } else {
            showToast(`Failed to add waypoint: ${data.message}`, "error");
        }
    })
    .catch(error => {
        console.error("Add waypoint fetch error:", error);
        showToast(`Error adding waypoint: ${error.message}`, "error");
    });
}
